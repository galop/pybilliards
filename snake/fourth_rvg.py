...
def gameLoop():
    ...
    while not gameExit:
        ...
        # after the snake crosses it. :D
        # Snake will just come from other side, after it crosses the boundary :D
        if lead_x >= dispWidth or lead_x < 0 or lead_y >= dispHeight or lead_y < 0:
            # gameOver = True
            lead_x = lead_x % dispWidth
            lead_y = lead_y % dispHeight
        lead_x += lead_x_del
        lead_y += lead_y_del
        ...
        print "before this ", snakeList
        if len(snakeList) > snakeLength:
            print "sorry I had to delete", len(snakeList)
            del snakeList[0]
        print "after this ", snakeList
        for anyElem in snakeList[:-1]:
            if anyElem == snakeHead:
                gameOver = True
